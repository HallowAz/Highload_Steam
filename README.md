...
Аналогом могла быть рассмотрена технология Latency-based DNS, но такой подход показался менее эффективным, поскольку в случае изменения временных задержек до ДЦ, поменяется выбор конкретного ДЦ маршрутизатором, а следоавтельно и состояние сессии клиента, которое было достигнуто на предыдущем ДЦ.
